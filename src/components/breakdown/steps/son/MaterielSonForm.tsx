...
import { MaterielSon, EquipeTechnique } from '@/lib/types-clean'
...
export interface MaterielSonFormRef {
  submitForm: () => Promise<void>
}

export const MaterielSonForm = forwardRef<MaterielSonFormRef, MaterielSonFormProps>(
  ({ sequenceId, materiel, onCancel, onSuccess }, ref) => {
    ...
    const { createMaterielSon, updateMaterielSon } = useMaterielSon(sequenceId)
    ...
    const submitForm = async () => {
      ...
      try {
        if (materiel) {
          // Modification
          const success = await updateMaterielSon(materiel.id, formData)
          if (success) {
            toast.success('Matériel son modifié avec succès', {
          ...
          // Création
          const newMateriel = await createMaterielSon({
          ...
          if (newMateriel) {
            toast.success('Matériel son créé avec succès', {
              style: { background: '#374151', color: '#ffffff' }
            })
            onSuccess()
          } else {
...
MaterielSonForm.displayName = 'MaterielSonForm'
